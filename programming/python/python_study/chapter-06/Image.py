...
class Image:
    ...
    def resize(self, width=None, height=None):
        #have to put in logic to cater for null values
        # and return a bool indicating changed or not
        if (self.__width == width) and (self.__height == height):
            ...
            self.__width = width
            self.__height = height
            # creating a new list containing all the coordinates of the matrix
            newcoords = []
            for x in range(width):
                 for y in range(height):
                     newcoords.append((x,y))
 
            #intermediary list created because i cant delete an item from a 
            #collection whilst itterating over the collection 
            self.__old_keys = []
            for key in self.__data.keys():
                self.__old_keys.append((key))
            """itterating over the intermediary list, if a value is not in the
            newcoords list that contains all the cooordinates of the resized
            matrix then the item is deleted from the true dict self.__data """
            for key in self.__old_keys:
                if key in newcoords:
                     pass
                     #print("save   ",key)
                else:
            ...
            #reset colors and populate with the remaining values 
            self.__colors = {self.__background}  
            for color in self.__data.values():
